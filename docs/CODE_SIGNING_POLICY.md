...
This document describes who can put code into AI RTL Fixer, who can authorise a signature over the resulting binary, and what a user can verify about a file they downloaded.
...
AI RTL Fixer is maintained by one person, so the same name appears in every role. Naming them anyway is the point of this document: it says exactly who can cause a signature to exist.
...
Source code: <https://github.com/miladateight/AI.RTL.Fixer>

Everything that ends up in a release is built from that repository. The project has no proprietary components, no commercial edition, and no dual licence: it is MIT throughout. Third-party dependencies and their licences are listed in the SBOM published with each release.

## Privacy policy

<https://github.com/miladateight/AI.RTL.Fixer/blob/main/docs/PRIVACY.md>
...
2. The workflow refuses the tag unless it matches the version declared in `Directory.Build.props` and `installer/AI.ChatRTLFixer.iss`.
3. The solution is built and the full test suite runs on a clean checkout.
4. The installer is compiled with Inno Setup from the self-contained build.
...
9. The release is published with the installer, its checksums, and the SBOM.
...
```powershell
Get-AuthenticodeSignature .\AIChatRTLFixerSetup-1.1.2.exe | Format-List
```
