...
.DESCRIPTION
    Produces dist\msix\AIRTLFixer-<version>.msix from
    dist\portable-self-contained-win-x64, using installer\msix\AppxManifest.template.xml
    and tile images generated from assets\branding\app-logo.png.
    ...
    Two different packages come out of this script depending on where it is going:
      ...
      Sideload (-SelfSign) signed with a local self-signed certificate so the
                         ...
                         written to dist\msix\, must be trusted manually, and
...
.PARAMETER SkipPublish
    Reuse the existing self-contained output instead of rebuilding it.
#>
param(
    [string] $IdentityName = "PLACEHOLDER.AIRTLFixer",
...
# ---------------------------------------------------------------- version ----
# Same source of truth as everything else (docs\RELEASE.md). MSIX wants four
# parts and the Store requires the last to be 0, so 1.1.2 becomes 1.1.2.0.
...
$payload = Join-Path $root "dist\portable-self-contained-win-x64"
if (-not (Test-Path (Join-Path $payload "AI.ChatRTLFixer.Tray.exe"))) {
    throw "Self-contained build not found in $payload. Run scripts\publish.ps1 first."
}
...
$logoPath = Join-Path $root "assets\branding\app-logo.png"
...
if ($SelfSign -and $isPlaceholder) {
    ...
    $IdentityName = "Ateight.AIRTLFixer.Test"
...
$package = Join-Path $outDir "AIRTLFixer-$packageVersion$suffix.msix"
